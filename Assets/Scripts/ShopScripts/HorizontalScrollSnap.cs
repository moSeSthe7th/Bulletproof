...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        void Update()
        {
            ...
            if(CurrentScreen() == 0 && PrevButton.GetComponent<Image>().enabled){
                PrevButton.GetComponent<Image>().enabled = false;
            }else if(CurrentScreen() >= _screens - 1 && NextButton.GetComponent<Image>().enabled){
                NextButton.GetComponent<Image>().enabled = false;
            }
            if(!PrevButton.GetComponent<Image>().enabled && CurrentScreen() != 0){
                PrevButton.GetComponent<Image>().enabled = true;
            }else if(!NextButton.GetComponent<Image>().enabled && CurrentScreen() != _screens - 1){
